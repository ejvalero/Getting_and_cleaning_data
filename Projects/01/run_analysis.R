preprocessing <- function(directory, tfiles, out = paste("main.", tfiles, sep = "")){
    # Uploading training and testing data to R
    path = paste(directory,"/", tfiles, sep = "")
    for(i in dir(path, pattern = ".txt", full.names = T)){
        data <- read.table(i)
        dataname <- strsplit(i, "/")[[1]][3]
    ...
    for (i in 1:length(all.data$Activity)){
        all.data$Activity[i] <- switch(all.data$Activity[i],
